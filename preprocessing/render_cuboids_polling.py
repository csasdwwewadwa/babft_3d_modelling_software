...
def reload_cuboids(instance_vbo):
    """
    Attempts to read cuboids_data.npy and uploads it to the provided OpenGL VBO.
    Returns the number of instances loaded, or None if the loading failed.
    """
    if not os.path.exists("cuboids_data.npy"):
        print("Warning: 'cuboids_data.npy' not found.")
        return None
    ...
    try:
        ...
        # Bind the buffer and overwrite its storage with the new data
        glBindBuffer(GL_ARRAY_BUFFER, instance_vbo)
        glBufferData(GL_ARRAY_BUFFER, buffer_dump.nbytes, buffer_dump, GL_DYNAMIC_DRAW)
        glBindBuffer(GL_ARRAY_BUFFER, 0)
        
        print(f"Reloaded {num_instances} instances.")
        return num_instances

    except Exception as e:
        # Handles potential read collisions if another script is writing to the file
        print(f"Failed to reload 'cuboids_data.npy': {e}")
        return None
...
def main():
    ...
    num_instances = reload_cuboids(InstanceVBO)
    if num_instances is None:
        num_instances = 0
    
    # Configure instance attribute pointers
    ...
    while not glfw.window_should_close(window):
        ...
        if need_reload:
            updated_count = reload_cuboids(InstanceVBO)
            if updated_count is not None:
                num_instances = updated_count
            need_reload = False
